...
subplot(2,2,3)
plot(((1:Lmax)*t0)/48,real(Hx(1:Lmax,7))*H0);hold on;
plot(((1:Lmax)*t0)/48,real(Hy(1:Lmax,7)*H0),'r');
...
hold on
for tind=1:Lmax
plot([real(Hx(tind,7)) real(Hx(tind+1,7))]*H0,[real(Hy(tind,7)) real(Hy(tind+1,7))]*H0)
%axis([-0.1 0.1 -0.1 0.1]);
end
...
Lmax=1024;
L2=1024;
Hx=[zeros(L2-Lmax,13); Hx((4800-Lmax+1):4800,:)]; 
Hy=[zeros(L2-Lmax,13); Hy((4800-Lmax+1):4800,:)]; 
Hz=[zeros(L2-Lmax,13); Hz((4800-Lmax+1):4800,:)]; 
Ex=[zeros(L2-Lmax,13); Ex((4800-Lmax+1):4800,:)]; 
Ey=[zeros(L2-Lmax,13); Ey((4800-Lmax+1):4800,:)]; 
Ez=[zeros(L2-Lmax,13); Ez((4800-Lmax+1):4800,:)]; 
