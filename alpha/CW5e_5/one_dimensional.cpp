...
int main(int argc, char **argv) {
  ...
  grid_volume gv = vol3d(1.0,1.0,24.0, a);
  gv.center_origin();
  structure s(gv, one, pml(1.0, Z), identity());
  s.add_susceptibility(two, H_stuff, gyrotropic_susceptibility(vec(0.0,0.0,1.0),1.0,0.001,0.00005,GYROTROPIC_SATURATED));
...
for (double fp=0.0;fp<=2.0;fp=fp+0.0001)
  ...
  f.add_point_source(Hz,src,vec(0.0,0.0,0.0));
...
*/
	...
	for (double ind=-12.0;ind<=12.0;ind=ind+1)    
		{
    monitor_point pin;
        f.get_point(&pin, vec(0.0,0.0,ind));
        cdouble E1i = pin.get_component(Ex);
